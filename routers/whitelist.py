...
@router.post("/create")
async def create(createWhiteList: CreateWhiteListModel, request: Request, background_tasks: BackgroundTasks,
...
async def get_my_whitelists(status: WhitelistStatus, request: Request, page: int = 1, limit: int = 12,
                            sort="firstModified", populate="true", token: str = Header(None)):
    userId = request.headers["userId"]
    whiteListQuery = {}
    if status == "accepted":
        whiteListQuery = {"$or": [{"partyA": userId}, {"partyB": userId}],
                          "$and": [{"partyAAccepted": True}, {"partyBAccepted": True}]}
    else:
        whiteListQuery = {"$or": [{"partyA": userId}, {"partyB": userId}],
                          "$or": [{"partyAAccepted": False}, {"partyBAccepted": False}]}
    whiteListDatas = whiteList_model_dal.read(query=whiteListQuery, limit=limit, page=page, sort=sort,
                                              populate=populate)
    return whiteListDatas


@router.get("/allFindWhitelistRequest")
async def get_my_whitelist_Request(request: Request, responded: bool = False, page: int = 1, limit: int = 12,
                                   sort="firstModified", populate="true", token: str = Header(None)):
    userId = request.headers["userId"]
    whiteListQuery = {"$or": [{"partyA": userId}, {"partyB": userId}], "$and": [{"responded": responded}]}
    whiteListDatas = whiteList_model_dal.read(query=whiteListQuery, limit=limit, page=page, sort=sort,
                                              populate=populate)
    return whiteListDatas
